...
class VideoHeroBlock {
  ...
  async initializeVideos() {
    ...
    try {
      ...
      if (window.videoHeroBlockData) {
        const blockId = this.blockElement.id || Object.keys(window.videoHeroBlockData)[0];
  ...
  resetVideoIndicator(index) {
    const indicators = this.blockElement.querySelectorAll(".ad-video-hero-block__video-indicator");
    if (indicators[index]) {
      const progressFill = indicators[index].querySelector(".ad-video-hero-block__progress-fill");
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].killTweensOf(progressFill);
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(progressFill, {
        width: "0%"
      });
  ...
  startProgressAnimation(index) {
    ...
    if (indicators[index]) {
      ...
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].fromTo(progressFill, {
        width: "0%"
      }, {
        width: "100%",
  ...
  createTextContent(videoIndex) {
    ...
    const contentPlaceholder = this.blockElement.querySelector(".ad-video-hero-block__video-content-placeholder");
  ...
  startTitleScrolling() {
    const titleScrollers = this.blockElement.querySelectorAll(".ad-video-hero-block__video-content.active .ad-video-hero-block__video-title-scroller .scrolling-text");
    titleScrollers.forEach(scrollingText => {
      scrollingText.dataset.scrollingStarted = "true";
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].killTweensOf(scrollingText);
      let originalText;
      ...
      if (this.blockElement) {
        ...
        if (window.innerWidth <= 480) {
          spacing = parseInt(computedStyle.getPropertyValue("--title-scrolling-gap-mobile")) || 150;
        } else if (window.innerWidth <= 768) {
          spacing = parseInt(computedStyle.getPropertyValue("--title-scrolling-gap-tablet")) || 200;
        } else {
          spacing = parseInt(computedStyle.getPropertyValue("--title-scrolling-gap")) || 300;
        }
      }
      ...
      const baseSpeed = this.blockElement ? parseInt(getComputedStyle(this.blockElement).getPropertyValue("--title-scrolling-speed")) || 100 : 100;
      const duration = Math.max(animationDistance / baseSpeed, 2);
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(scrollingText, {
        x: 0
      });
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(scrollingText, {
        x: -animationDistance,
        duration: duration,
        ease: "none",
        repeat: -1,
  ...
  stopTitleScrolling() {
    const scrollingTexts = this.blockElement.querySelectorAll(".scrolling-text");
    scrollingTexts.forEach(text => {
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].killTweensOf(text);
  ...
  animateVideoTransition(currentVideoSlide, newVideoSlide, currentDirection, currentVideo, previousVideoIndex) {
    ...
    gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(newVideoSlide, {
      y: newVideoStartY,
      opacity: 0,
      scale: 1
    });
    ...
    let currentTextContent = this.blockElement.querySelector(`.ad-video-hero-block__video-content[data-video-index="${previousVideoIndex}"]`);
    let newTextContent = this.blockElement.querySelector(`.ad-video-hero-block__video-content[data-video-index="${this.currentVideoIndex}"]`);
    if (!currentTextContent) {
      currentTextContent = this.createTextContent(previousVideoIndex);
      if (currentTextContent) {
        gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(currentTextContent, {
          y: "0%",
          opacity: 1
        });
      }
    ...
    if (!newTextContent) {
      ...
      if (newTextContent) {
        gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(newTextContent, {
          y: newVideoStartY,
          opacity: 0
        });
      }
    ...
    if (currentTextContent && newTextContent) {
      tl.to(currentTextContent, {
        y: currentVideoExitY,
        opacity: 0,
        duration: 1.2,
        ease: "power3.inOut"
      }, 0).fromTo(newTextContent, {
        y: newVideoStartY,
        opacity: 0
      }, {
        y: "0%",
        ...
        setTimeout(() => {
          if (currentTextContent && currentTextContent.parentNode) {
            const allTextElements = this.blockElement.querySelectorAll(".ad-video-hero-block__video-content");
  ...
  changeVideo(isAutomatic = true, targetIndex = null, directionOverride = null) {
    ...
    if (targetIndex !== null) {
      ...
      currentDirection = directionOverride || (isAutomatic ? "next" : this.direction);
    ...
    const currentVideoSlide = this.blockElement.querySelector(`.ad-video-hero-block__video-slide[data-video-index="${previousVideoIndex}"]`);
    const newVideoSlide = this.blockElement.querySelector(`.ad-video-hero-block__video-slide[data-video-index="${this.currentVideoIndex}"]`);
    ...
    this.animateVideoTransition(currentVideoSlide, newVideoSlide, currentDirection, currentVideo, previousVideoIndex);
    if (this.autoPlay) {
  ...
  createVideoSlides() {
    const videoBackground = this.blockElement.querySelector(".ad-video-hero-block__video-background");
    if (!videoBackground || this.videos.length === 0) return;
    videoBackground.innerHTML = "";
    this.videos.forEach((video, index) => {
      const embedUrl = this.getEmbedUrl(video.videoUrl, video.videoType, video.autoplay, video.muted);
      const videoSlide = document.createElement("div");
      videoSlide.className = `ad-video-hero-block__video-slide ${index === 0 ? "active" : ""}`;
      ...
      `;
        ...
        font-size: 24px;
      `;
        placeholder.textContent = `${video.videoType === "youtube" ? "📺" : "🎬"} ${video.title}`;
  ...
  createVideoContent() {
    ...
    this.videos.forEach((video, index) => {
      ...
      contentDiv.className = `ad-video-hero-block__video-content ${index === 0 ? "active" : ""}`;
  ...
  initializeVideoIndicators() {
    if (window.videoHeroBlockData) {
      const blockId = this.blockElement.id || Object.keys(window.videoHeroBlockData)[0];
      const indicatorsContainer = this.blockElement.querySelector(".ad-video-hero-block__video-indicators");
      const progressIndicatorsContainer = this.blockElement.querySelector(".ad-video-hero-block__progress-indicators");
      if (!indicatorsContainer || !progressIndicatorsContainer || this.videos.length <= 1) return;
      const existingIndicators = this.blockElement.querySelectorAll(".ad-video-hero-block__video-indicator");
      existingIndicators.forEach(indicator => indicator.remove());
      this.videos.forEach((video, index) => {
        const indicator = document.createElement("div");
        indicator.classList.add("ad-video-hero-block__video-indicator");
        indicator.style.setProperty("width", `${100 / window.videoHeroBlockData[blockId].videos.length - 2}vw`, "important");
  ...
  initializeFirstVideo() {
    if (this.videos.length === 0) return;
    const firstVideoSlide = this.blockElement.querySelector('.ad-video-hero-block__video-slide[data-video-index="0"]');
    if (firstVideoSlide) {
      firstVideoSlide.classList.add("active");
      gsap__WEBPACK_IMPORTED_MODULE_0__["default"].set(firstVideoSlide, {
        opacity: 1
      });
    }
    const firstTextContent = this.blockElement.querySelector('.ad-video-hero-block__video-content[data-video-index="0"]');
    if (firstTextContent) {
  ...
  startVideoSlider() {
    ...
    if (this.autoPlay) {
      this.videoTimeout = setTimeout(() => this.changeVideo(true), this.transitionDuration);
    }
    this.startProgressAnimation(this.currentVideoIndex);
  }
  setupEventListeners() {
    ...
    this.blockElement.addEventListener("mousemove", event => {
      const container = this.blockElement.querySelector(".ad-video-hero-block__container");
      ...
      const isOverIndicator = target.closest(".ad-video-hero-block__video-indicator");
      ...
      const newDirection = event.clientX - rect.left < viewportWidth / 2 ? "prev" : "next";
      const newText = newDirection === "prev" ? "<" : ">";
      if (newDirection !== this.direction || this.cursorText.textContent !== newText) {
        this.direction = newDirection;
        gsap__WEBPACK_IMPORTED_MODULE_0__["default"].to(this.cursorText, {
          opacity: 0,
          duration: 0.2,
          ease: "power1.out",
          onComplete: () => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  videoHeroBlocks.forEach((blockElement, index) => {
    ...
    const cursor = blockElement.querySelector(".ad-video-hero-block__video-cursor");
    const cursorText = cursor ? cursor.querySelector(".ad-video-hero-block__cursor-text") : null;
